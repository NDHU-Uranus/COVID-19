...
# reapet v times 
  system.time({n.times_WHM.kfoldcv <- n.times_tempWHM.kfoldcv(10)})
...
k_folds_lm.ori.cv <- function(fold,bbaa){
  k_fold_mse <-c()
  k_fold_mae <-c()
  #set.seed(6)
  cv  <- crossv_kfold(data = CA_case[,1:3], k = fold ) # k-fold cross-Validation
...
# reapet v times 
system.time({n.times_LM.kfoldcv <- n.times_tempLM.kfoldcv(10)})
...
# reapet v times 
system.time({n.times_GLM.kfoldcv <- n.times_tempGLM.kfoldcv(10)})
...
# reapet v times 
system.time({n.times_GAM.kfoldcv <- n.times_tempGAM.kfoldcv(10)})
...
par(mfrow = c(1, 3))
# k fold mse
plot(n.times_WHM.kfoldcv[[1]], ylim = c(0.5,1.5),ylab="MSE",
     type = "b",col = "turquoise", pch = 20,main = "rep 10 times MSE")
...
# k fold RMSE
plot(n.times_WHM.kfoldcv[[2]], ylim = c(0.5,1.5),ylab="RMSE",
...
# k fold MAE
plot(n.times_WHM.kfoldcv[[3]], ylim = c(0.5,1.5),ylab="MAE",
