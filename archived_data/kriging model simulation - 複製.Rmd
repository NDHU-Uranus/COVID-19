...
#single Diagnosis rate(單日確診率)
CA_case$single_diagnosis_rate <- as.numeric(CA_case$reported_cases/CA_case$population)
#hospital beds rate(病床持有率)
CA_case$Hospital_beds <- as.numeric(vari_bed$BEDS/sum(vari_bed$BEDS))
...
califor_418diagnosis_map <- ca_base + 
  geom_polygon(data =  ca_fact, aes(fill = single_diagnosis_rate), color = "white") +
  geom_polygon(color = 'pink', fill = NA) +
  scale_fill_gradientn(colours = plasma(7))  +
  labs(title = 'califor 4/18 diagnosis rate') + theme(plot.title = element_text(hjust = 0.5))
...
predict_map <- ca_base + 
  geom_polygon(data =  simu_krig_data, aes(fill = simu_y_hat), color = "blue") +
  geom_polygon(color = 'pink', fill = NA) +
  scale_fill_gradientn(colours = plasma(7))  +
  labs(title = 'califor cumulative confirmed case') + theme(plot.title = element_text(hjust = 0.5))
predict_map
...
Real data case (contain all variables(rate)): COVID-19 diagnosis rate  data in California 
```{r, echo=FALSE, warning=FALSE,message=FALSE}
#rm(list = ls())
...
# combine all variables
intere_vair <- cbind(vari_unemp,CA_case$single_diagnosis_rate,vari_pover[,2],vari_educa[,2],vari_bed[,2],vari_houseunits[,2:4]) 
...
Real data case (only important variables(rate)): COVID-19 diagnosis rate  data in California 
```{r, echo=FALSE, warning=FALSE,message=FALSE}
#rm(list = ls())
