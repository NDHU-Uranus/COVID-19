...
CA_case$Hospital_beds <- scale(as.numeric(vari_bed$BEDS/sum(vari_bed$BEDS)))
...
CA_case <- cbind(CA_case[,c(4,5,40:45)],CA_pm2.5_total_KNNout[,4])
...
k_folds_WHM.cv <- function(fold,bbaa){
  ...
  for (t in 1:length(cv$.id)) {
    ...
    simubas <- as.matrix(cbind(mrts(knot = case_train[1:2], bbaa),case_train$Hospital_beds
                               ,case_train$Unemployment,case_train$Poverty,case_train$Education
                               ,case_train$population_rate,case_train$`CA_pm2.5_total_KNNout[, 4]`))
    ...
    WHM_p <- whm(response.variable, CA_case_Design.matrix[,-1], var.function = "exponential", tuning.para = 1.345, ite = 500)
    WHM_p[[1]]
    ...
    simuvario.b <- variog(coords = case_train[1:2], data = covadata,uvec=seq(0,jj, l=400), max.dist=6)
    ...
    simu_b_grid <- as.matrix(cbind(mrts(knot = case_test[1:2], bbaa),case_test$Hospital_beds
                                   ,case_test$Unemployment,case_test$Poverty,case_test$Education
                                   ,case_test$population_rate,case_test$`CA_pm2.5_total_KNNout[, 4]`))
...
# reapet v times 
system.time({n.times_WHM.kfoldcv <- n.times_tempWHM.kfoldcv(repeat.time=10,fold=5,basis=3)})
...
# reapet v times 
system.time({n.times_LM.kfoldcv <- n.times_tempLM.kfoldcv(repeat.time=10,fold=5,basis=3)})
...
# reapet v times 
system.time({n.times_GLM.kfoldcv <- n.times_tempGLM.kfoldcv(repeat.time=10,fold=5,basis=3)})
...
# 
# # reapet v times 
# system.time({n.times_GAM.kfoldcv <- n.times_tempGAM.kfoldcv(repeat.time=10,fold=5,basis=3)})
# 
# par(mfrow = c(1, 3))
# # k fold mse
...
par(mfrow = c(1, 3))
# k fold mse
plot(n.times_WHM.kfoldcv[[1]], ylim = c(0.2,0.8),ylab="MSE",
     type = "b",col = "turquoise", pch = 20,main = "rep 10 times MSE")
...
# k fold RMSE
plot(n.times_WHM.kfoldcv[[2]], ylim = c(0.2,0.8),ylab="RMSE",
...
# k fold MAE
plot(n.times_WHM.kfoldcv[[3]], ylim = c(0.2,0.8),ylab="MAE",
